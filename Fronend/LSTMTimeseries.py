...
samples = st.number_input('Choose Number of Samples;',1)
#st.write('The current number2 is ', number2)
#number1=int(number1)
#number2=int(number2)
init=st.slider("Initial Sample index:",0,samples )    
final=st.slider("Final Sample index:",0,samples )  
dif=final-init+1
st.write("Window Size:",dif)
...
plt.plot(G_TIME, G_SERIES, label="Selected time series",linewidth=5)
plt.scatter(G_MAX+1, pred, color="red", marker="x", s=100, label="prediction")
plt.scatter(G_MAX+1, G.SERIES[final+1], color="blue", marker="o", s=100, label="Original")
plt.legend()
